...
def get_git_status(args: Optional[str] = "") -> Dict[str, str]: 
    """`git status`: returns the current working tree status (staged, unstaged, untracked). Use args to filter."""
    pass
...
def get_git_branch(args: Optional[str] = "") -> Dict[str, str]: 
    """`git branch`: lists branches in the local repository. Use args to specify options like '-a'."""
    pass

@tool("read")
@bash_wrap("git_log", ["git", "log"])
def get_git_log(args: Optional[str] = "") -> Dict[str, str]: 
    """`git log`: shows the commit history. Use args for formatting or limiting results (e.g., '--oneline')."""
    pass
